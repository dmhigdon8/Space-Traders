...
#there's some metadata I'm not currently using, but it's there if I need it
print(response.text)
...
if response.status_code == 200:
    ...
    for waypoint in waypoints:
        ...
        print(f" ---End Modifiers---\n")

        #Begin Chart
        print("   -----Chart-----")
        chart = waypoint.get("chart", {})
        chart_submittedby = chart.get("submittedBy")
        chart_submitted_date = chart.get("submittedOn")
        print(f"  Submitted By: {chart_submittedby}\n"
              f"  Submitted On: {chart_submitted_date}\n"
              f"  ---End Chart---\n")
